...
Fsublarval1<-subset(Fsublarval1, F_female_survived_laying1==1)
...
TFL_emergence_m_f <- ggplot(prop_emerged_all_male_female, aes(x=temp, y=`mean(emerged)`, group=emerg_male_female, ymin=lower, ymax=upper)) +
                       ...
                       geom_errorbar(position=pd3, size=0.5, alpha=.4,width=.15, aes(colour=factor(emerg_male_female)))+ #use this for error bars
...
#Saving high res plot
png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_male_female_fertility_error.png", type="cairo", units="in", width=10, height=6, pointsize=10, res=1000)
...
#Saving high res plot of both male and female survival
png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_survival_all.png", type="cairo", units="in", width=11, height=4, pointsize=10, res=1000)
grid.arrange(M_survival, F_survival_FL, ncol=2)
dev.off() 
...
#png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_all_offspring.png", type="cairo", units="in", width=10, height=6, pointsize=10, res=1000)
#print(all_offspring)
#dev.off() 
...
#### 16. Density plot of pupation time ####

sub_pupation<- subset(pupaeTFL, pupation !='NA')

pupation.length<-ggplot(sub_pupation, aes(x=pupation, group=temp))+
                 geom_density(aes(colour=factor(temp)))+
                 theme_bw()

pupation.length


#### 17. Statsummary of pupation time ####


pupation.box<- ggplot(sub_pupation, aes(x=temp, y=pupation))+
                      stat_summary(fun.data= mean_se, geom="pointrange")+
                      stat_summary(fun.y = mean, geom="line")+
                      scale_x_continuous('Pupal heat shock temperature (°C)') +
                      scale_y_continuous('Pupation length') +
                      theme(panel.background = element_rect(fill = "white",colour = "gray90"),
                                axis.line=element_line(colour='gray90'),
                                axis.text=element_text(colour='black'),
                                panel.grid.major = element_line("gray90"),
...
submateb2<-subset(beh.mot,block==2)
submateb2<- subset(submateb2,mating_success ==1 |mating_success ==0)
submatefemale<- subset(submate, sex==0)
submatemale<- subset(submate, sex==1)
submatemaleb2<- subset(submateb2, sex==1)
...
prop_male_matingb2 <- summarise(group_by(submatemaleb2, temp), mean(mating_success), N=length(mating_success))
prop_male_matingb2 <-cbind((binom.confint(prop_male_matingb2$`mean(mating_success)`*prop_male_matingb2$N, prop_male_matingb2$N, conf.level = 0.95, methods = "logit")),prop_male_matingb2)
...
male_mating_errorb2<- ggplot(prop_male_matingb2, aes(x=temp, y=`mean(mating_success)`, ymin=lower, ymax=upper)) + 
  geom_point(position=pd, size=1.5, colour="black") +
  geom_errorbar(colour="black", position=pd, size=0.5, width=.2)+
  scale_x_continuous('Pupal heat shock temperature', breaks= round(seq(min(28), max(35), by = 1),1)) +
  scale_y_continuous('Proportion of treated males successfully mating')+
...
#### 3. Male and female mating with error ####

prop_all_mating <-as.data.frame(summarise(group_by(submate, temp, sex), mean(mating_success), N=length(mating_success)))

prop_all_mating<-cbind((binom.confint(prop_all_mating$`mean(mating_success)`*prop_all_mating$N, prop_all_mating$N, conf.level = 0.95, methods = "logit")),prop_all_mating)

pd9 <- position_dodge(0)
mating_success_m_f <- ggplot(prop_all_mating, aes(x=temp, y=`mean(mating_success)`, group=sex, ymin=lower, ymax=upper)) +
                       geom_point(position=pd9, size=1.5, aes(colour=factor(sex))) +
                      geom_errorbar(position=pd9, size=0.5, width=.15, aes(colour=factor(sex)))+ #use this for error bars
                       geom_line(position=pd9, aes(colour=factor(sex)))+
                       scale_x_continuous('Pupal heat-shock temperature (°C)',breaks = round(seq(min(28), max(35), by = 1),1)) +
                       scale_y_continuous('Proportion of focal individuals mating within 3 hours')+
                       coord_cartesian(ylim=c(0, 1)) +
...
#png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_mating_success_m_f.png", type="cairo", units="in", width=6, height=6, pointsize=10, res=1000)
#print(mating_success_m_f)
#dev.off() 

#### 4. 3 day motility with error ####
...
#Saving high res plot
png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_motility_all_error.png", type="cairo", units="in", width=8, height=6, pointsize=10, res=1000)
print(motility_all_error)
dev.off() 

#### 7. Motility of block 5 IN pupaeTFL dataset ####
...
#### 8. Female mating latency temp as a numeric ####
...
#### 9. Female mating latency temp as a factor ####
...
#### 10. Female mating duration temp as a factor ####
submatefemale$temp<-as.factor(submatefemale$temp)
#sub_all_adults$temp<-as.numeric(sub_all_adults$temp)
submatefemale$mating_s<-as.numeric(submatefemale$mating_s)
...
mating_duration_F_factor <- ggplot(submatefemale, aes(x=temp, y=duration_s))+
                            ...
                            geom_boxplot(outlier.colour=NA, alpha=.5, width=.2, aes(fill=sex), show.legend = FALSE) +
                            scale_x_discrete('Pupal heat-shock temperature of females (°C)') +
                            scale_y_continuous('Mating duration (s)', breaks= round(seq(min(0), max(9000), by = 200),1))+
                            #guides(color=guide_legend("Sex"))+
                            theme(panel.background = element_rect(fill = "white",colour = "gray90"),
                                  axis.line=element_line(colour='gray90'),
                                  axis.text=element_text(colour='black'),
...
#Saving high res plot of both latency and duration
png(filename="C:/Users/bwalsh/Documents/R docs/D-mel_latency_duration.png", type="cairo", units="in", width=10, height=6, pointsize=10, res=1000)
grid.arrange(mating_latency_F_factor, mating_duration_F_factor, ncol=2)
dev.off() 
